...
export const getFirstStep = (steps: Steps): IStep | null =>
  steps &&
  Object.values(steps).reduce((a: IStep | null, b) => (!a || a.order > b.order ? b : a), null);
...
export const getStepNumber = (steps: Steps, step?: IStep): number | undefined =>
  step && Object.values(steps).filter((_step) => _step.order <= step.order).length;
...
export const getNextStep = (steps: Steps, step?: IStep): IStep | null | undefined =>
  Object.values(steps)
    .filter((_step) => _step.order > step!.order)
    .reduce((a: IStep | null, b) => (!a || a.order > b.order ? b : a), null);
...
const getBorderRadiusOrDefault = (borderRadius?: number, defaultRadius: number = 0) =>
...
export const defaultSvgPath = ({
  ...
  if (radius || borderRadiusObject) {
    const borderRadiusTopRight = getBorderRadiusOrDefault(borderRadiusObject?.topRight, radius);
    const borderRadiusTopLeft = getBorderRadiusOrDefault(borderRadiusObject?.topLeft, radius);
    const borderRadiusBottomRight = getBorderRadiusOrDefault(
      borderRadiusObject?.bottomRight,
      radius
    );
    const borderRadiusBottomLeft = getBorderRadiusOrDefault(borderRadiusObject?.bottomLeft, radius);
    ...
    return `M${position.x},${position.y}H${
      ...
      position.y +
      (borderRadiusBottomLeft > borderRadiusTopLeft ? borderRadiusTopLeft : borderRadiusBottomLeft)
  ...
  (
    ...
    switch (shape) {
      ...
      case 'circle_and_keep': {
        ...
        return separate(
          previousPath,
          [path, circleSvgPath({ size: sizeOffset(size, maskOffset), position })],
          optionsKeep
        );
      }
